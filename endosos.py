...
def to_latin1(text): return clean_text(text).encode('latin1', 'replace').decode('latin1')

Función para determinar el color de una celda en base al porcentaje
def get_color(similarity_percentage): if similarity_percentage < 89: return (255, 0, 0) # Rojo elif 89 <= similarity_percentage <= 92: return (255, 255, 0) # Amarillo else: return (255, 255, 255) # Blanco (Defecto)
...
image_path = 'interesse.jpg' image = Image.open(image_path) st.image(image, caption='Interesse', use_column_width=True)
...
if uploaded_file_1 and uploaded_file_2: text_by_code_1 = extract_and_clean_text(uploaded_file_1) text_by_code_2 = extract_and_clean_text(uploaded_file_2)
