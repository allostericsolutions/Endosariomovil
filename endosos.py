...
def extract_and_clean_text(pdf_path):
    raw_text = extract_text(pdf_path)import streamlit as st
from pdfminer.high_level import extract_text
from fpdf import FPDF
import pandas as pd
import io
...
1. **Remoción de Expresiones Comunes**:
   - Ajuste en los patrones para eliminar correctamente "02001/M" seguido de cualquier dígito.

2. **Estilo CSS**:
   - Agregué un bloque CSS en la variable `table_styles` para asegurar que las columnas tengan el mismo ancho.
      - `.comparison-wrapper` es un contenedor `flex` que estructura las columnas.
      - `.comparison-wrap` define la anchura de las columnas del 45% cada una.

3. **HTML para Comparación**:
   - Se generaron dos columnas a partir del DataFrame.
   - Se combinó el CSS y HTML para ajustar el diseño de la tabla y las columnas.

4. **Mostrar Tabla en Streamlit**:
   - El HTML combinado con CSS se muestra utilizando `st.markdown()` para permitir una visualización equilibrada.
